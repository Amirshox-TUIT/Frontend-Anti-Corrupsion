...
export default function AdminDashboard() {
  ...
  const openReport = (report) => {
    ...
    setDecisionMessage('');
    setActionError('');
    setRewardAmount(report.rewardInfo?.amount || '');
    setRewardError('');
    setRewardSuccess('');
  };
  ...
  // ── Reward assign handler ─────────────────────────────────────────────────
  const submitRewardAssign = async () => {
    if (!selectedReport || !rewardAmount) return;
    const amount = parseFloat(rewardAmount);
    if (isNaN(amount) || amount < 1) {
      setRewardError(t('admin.errors.invalidRewardAmount', 'Minimum reward is 1 USDT'));
      return;
    }
    ...
    try {
      const response = await adminService.assignReward(selectedReport.id, amount);
      const decorated = decorateReport(response, i18n.resolvedLanguage, t);
      setSelectedReport(decorated);
      setRewardSuccess(t('admin.reward.assignSuccess', 'Reward assigned successfully'));
      await loadReports({ showLoading: false });
    } catch (error) {
      setRewardError(t(error?.message || 'admin.errors.rewardAssignFailed', 'Failed to assign reward'));
    } finally {
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex flex-col gap-3 md:flex-row">
              <select
                  ...
                  onChange={(e) => setFilters((c) => ({ ...c, regionId: e.target.value }))}
              ...
              <input
                  ...
                  onChange={(e) => setFilters((c) => ({ ...c, organization: e.target.value }))}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200">
              ...
              <tr className="text-left text-xs uppercase tracking-[0.18em] text-slate-500">
                ...
                <th className="px-6 py-4">{t('admin.table.reward')}</th>
                          ...
                          <td className="px-6 py-4">
                            ...
                            <span
                                className={`h-1.5 w-1.5 rounded-full ${
                                    report.rewardInfo.claimed ? 'bg-emerald-500' : 'bg-amber-500'
                                }`}
                            />
                                  {report.rewardInfo.amount} USDT
                                  {report.rewardInfo.claimed
                                      ? ` · ${t('admin.reward.claimed', 'Claimed')}`
                                      : ` · ${t('admin.reward.pending', 'Unclaimed')}`}
                          </span>
                            ) : (
        ...
        >
          {selectedReport ? (
              <div className="space-y-8">
                ...
                {/* ── Main grid: description + status actions ── */}
                ...
                {selectedReport.status === 'done' ? (
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="text-xs uppercase tracking-[0.22em] text-amber-700">
                            {t('admin.reward.sectionTitle', 'Whistleblower Reward')}
                          </p>
                          <p className="mt-1 text-sm text-slate-600">
                            {t(
                                'admin.reward.sectionHint',
                                'Set a USDT reward. The whistleblower can claim it anonymously using their tracking ID.',
                            )}
                          </p>
                        </div>
                        {/* Coin icon */}
                    ...
                    <svg viewBox="0 0 24 24" className="h-5 w-5 fill-current">
                      ...
                      {selectedReport.rewardInfo ? (
                          <div className="mt-4 rounded-2xl border border-amber-200 bg-white p-4">
                            <div className="flex flex-wrap items-center gap-3">
                      <span
                          className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${
                              selectedReport.rewardInfo.claimed
                                  ? 'bg-emerald-100 text-emerald-700'
                                  : 'bg-amber-100 text-amber-700'
                          }`}
                      >
                        <span
                            className={`h-1.5 w-1.5 rounded-full ${
                                selectedReport.rewardInfo.claimed ? 'bg-emerald-500' : 'bg-amber-500'
                            }`}
                        />
                        {selectedReport.rewardInfo.claimed
                            ? t('admin.reward.claimed', 'Claimed')
                        ...
                        {selectedReport.rewardInfo.amount} USDT
                      </span>
                              {selectedReport.rewardInfo.claimed && selectedReport.rewardInfo.wallet ? (
                                  <span className="font-mono text-xs text-slate-500">
                          → {selectedReport.rewardInfo.wallet}
                        </span>
                              ) : null}
                            </div>
                            {selectedReport.rewardInfo.claimed && selectedReport.rewardInfo.txHash ? (
                                <div className="mt-3 rounded-xl bg-slate-50 px-3 py-2">
                                  <p className="text-xs uppercase tracking-[0.16em] text-slate-500">
                                    {t('admin.reward.txHash', 'Transaction')}
                                  </p>
                                  <p className="mt-1 break-all font-mono text-xs text-slate-700">
                                    {selectedReport.rewardInfo.txHash}
                                  </p>
                                </div>
                            ) : null}
                            {selectedReport.rewardInfo.claimedAt ? (
                                <p className="mt-2 text-xs uppercase tracking-[0.16em] text-slate-400">
                                  {t('admin.reward.claimedAt', 'Claimed at')}:{' '}
                                  {formatDate(selectedReport.rewardInfo.claimedAt, i18n.resolvedLanguage)}
                                </p>
                            ) : null}
                          </div>
                      ) : null}

                      {/* Assign / update form — hidden if already claimed */}
                      {!selectedReport.rewardInfo?.claimed ? (
                          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
                            <div className="flex-1 space-y-2">
                              <label className="text-sm font-semibold text-slate-900">
                                {t('admin.reward.amountLabel', 'Reward amount (USDT)')}
                              </label>
                          ...
                          USDT
                            ...
                            <button
                                type="button"
                                onClick={submitRewardAssign}
                                disabled={rewardLoading || !rewardAmount}
                                className="action-primary shrink-0 disabled:cursor-not-allowed disabled:opacity-60"
                            >
                              {rewardLoading
                                  ? t('admin.reward.assigning', 'Assigning…')
                                  : selectedReport.rewardInfo
                                      ? t('admin.reward.updateBtn', 'Update reward')
                                      : t('admin.reward.assignBtn', 'Assign reward')}
                            </button>
                          </div>
                      ) : null}
                        ...
                        selectedReport.evidence.map((file) => (
                            <article key={file.id} className="overflow-hidden rounded-[24px] border border-slate-200">
                              <div className="aspect-[16/10] bg-slate-100">
                                {file.type.startsWith('image/') && file.previewUrl && !file.previewUrl.startsWith('blob:') ? (
                                    <img src={file.previewUrl} alt={file.name} className="h-full w-full object-cover" />
                                ) : file.type.startsWith('video/') && file.previewUrl && !file.previewUrl.startsWith('blob:') ? (
                              ...
                              <div className="p-4">
                                ...
                                <p className="mt-1 text-xs uppercase tracking-[0.16em] text-slate-500">{file.type}</p>
                              </div>
                            </article>
                        ))
                    ) : (
